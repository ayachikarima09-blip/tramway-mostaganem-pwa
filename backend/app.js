...
// ==================== LISTE NOIRE SUPPRESSIONS HORS LIGNE ====================
...
function addPendingDelete(mongoId) {
    const list = getPendingDeletes();
    if (!list.includes(mongoId)) {
        list.push(mongoId);
        localStorage.setItem('pendingDeletes', JSON.stringify(list));
    }
}

function removePendingDelete(mongoId) {
    const list = getPendingDeletes().filter(id => id !== mongoId);
    localStorage.setItem('pendingDeletes', JSON.stringify(list));
}
        ...
        try {
            ...
            if (res.ok) {
                ...
                for (const mongoObs of mongoData) {
                    ...
                    // Ignorer les observations supprimées hors ligne
                    const mongoId = (mongoObs._id || '').toString();
                    if (pendingDeletes.includes(mongoId)) {
                        console.log('🚫 Ignoré (supprimé hors ligne):', mongoId);
                        continue;
                    }
...
function generateEditForm(obs) {
    ...
    '<div class="form-group"><label>Atmosphère générale</label><textarea name="atmosphere_generale" placeholder="Décrire l\'atmosphère générale observée...">' + (obs.atmosphere_generale || '') + '</textarea></div>' +
    ...
    '<div class="form-group"><label>Perception patrimoine</label><textarea name="perception_patrimoine" placeholder="Décrire la perception du patrimoine...">' + (obs.perception_patrimoine || '') + '</textarea></div></div>' +
...
async function syncAll() {
    ...
    for (const obs of pending) {
        try {
            ...
            const hasValidId = isValidMongoId(obs._id);
            const method = hasValidId ? 'PUT' : 'POST';
            const url = hasValidId
                ? `${API_BASE}/api/observations/${obs._id.toString()}`
                : `${API_BASE}/api/observations`;
...
async function deleteObservation(id) {
    ...
    try {
        const mongoId = obs._id ? obs._id.toString() : null;
        const isOnline = await checkConnection();
        ...
        if (mongoId && isValidMongoId(mongoId)) {
            if (isOnline) {
                try {
                    ...
                    if (res.ok) {
                        removePendingDelete(mongoId);
                        console.log('✅ Supprimé sur MongoDB:', mongoId);
                    } else {
                        console.warn('Suppression MongoDB échouée:', res.status);
                        addPendingDelete(mongoId);
                    }
                } catch (err) {
                    console.warn('Erreur suppression MongoDB (hors ligne?):', err);
                    addPendingDelete(mongoId);
                }
            } else {
                // Hors ligne : mémoriser pour supprimer plus tard
                addPendingDelete(mongoId);
                console.log('📋 Suppression différée (hors ligne):', mongoId);
            }
        }
...
async function backgroundSync() {
    ...
    // Traiter les suppressions différées hors ligne
    const pendingDeletes = getPendingDeletes();
    for (const mongoId of pendingDeletes) {
        try {
            const res = await fetch(`${API_BASE}/api/observations/${mongoId}`, {
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' }
            ...
            if (res.ok) {
                removePendingDelete(mongoId);
                console.log('✅ Suppression différée exécutée:', mongoId);
            }
        } catch (err) {
            console.warn('Suppression différée échouée:', mongoId, err);
        }
    }
