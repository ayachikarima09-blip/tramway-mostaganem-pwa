...
async function handleNewObservationSubmit(event) {
    ...
    try {
        ...
        // ✅ Normaliser toutes les observations
...
function displayObservations(observations) {
    ...
    container.innerHTML = observations.map(obs => {
        // ✅ Convertir _id en string (ObjectId MongoDB peut être un objet)
        const obsId = obs.id || (typeof obs._id === 'object' ? obs._id.toString() : obs._id);
...
function viewDetails(id) {
    // ✅ Recherche robuste: gère ObjectId MongoDB (objet) et strings
    const obs = allObservations.find(o => {
        const mongoId = o._id ? (typeof o._id === 'object' ? o._id.toString() : String(o._id)) : null;
        const localId = o.id ? String(o.id) : null;
        const searchId = typeof id === 'object' ? id.toString() : String(id);
        return localId === searchId || mongoId === searchId;
    });
    if (!obs) return;
