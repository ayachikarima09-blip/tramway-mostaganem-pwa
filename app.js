    ...
    if (isOnline) {
        try {
            ...
            if (res.ok) {
                ...
                // Fusionner intelligemment : garder les versions locales si elles existent
                for (const mongoObs of mongoData) {
                    const localObs = allObservations.find(o => 
                        (o.id === mongoObs.id || o.id === mongoObs._id || o._id === mongoObs._id)
                    );
                    
                    // Si l'observation n'existe pas localement, l'ajouter
                    if (!localObs) {
                        if (!mongoObs.id) {
                            mongoObs.id = mongoObs._id || Date.now() + Math.random();
                        }
                        mongoObs.synced = true;
                        await saveObservation(mongoObs);
                    }
                    // Sinon, garder la version locale (ne pas écraser)
                }
...
async function handleEditSubmit(event) {
    ...
    // Marquer comme modifié et à synchroniser
    obs.synced = false;
