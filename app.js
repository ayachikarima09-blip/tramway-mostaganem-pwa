...
async function deleteObservation(id) {
    ...
    try {
        // Supprimer sur MongoDB si l'observation est synchronisée
        const isOnline = await checkConnection();
        if (isOnline && obs._id && /^[0-9a-fA-F]{24}$/.test(obs._id.toString())) {
            try {
                const res = await fetch(`${API_BASE}/api/observations/${obs._id.toString()}`, {
                    method: 'DELETE',
                    headers: { 'Content-Type': 'application/json' }
                });
                if (!res.ok) {
                    ...
                    console.log('✅ Supprimé sur MongoDB:', obs._id);
                }
            } catch (err) {
                console.warn('Erreur suppression MongoDB:', err);
                // On continue quand même pour supprimer localement
            }
        }
